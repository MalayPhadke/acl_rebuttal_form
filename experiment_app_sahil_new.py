...
QUESTIONS_CSV   = _find_file("list_questions_sahil_new.csv")
IMAGES_BASE_DIR = _find_dir("video_frames_sahil_new")

# Google Sheet worksheet names
SHEET_RESPONSE_METADATA = "seq_response_metadata_sahil_new"
SHEET_RESPONSE_SIMPLE = "seq_response_simple_sahil_new"
...
def video_path_to_folder(video_path, images_dir=None):
    """Convert video_path to the folder name used in the filtered images directory."""
    ...
    # Check for Videoframe_extract_unreviewed_XXXXX pattern
    if basename.startswith("Videoframe_extract_unreviewed_"):
        if images_dir and os.path.exists(os.path.join(images_dir, basename)):
            return basename
...
def load_questions_data(part, seed=None):
    ...
    for idx, row in df.iterrows():
        frames = parse_frames_list(row.get("frames", "[]"))
        ...
        trials.append(
            {
                ...
                "gt_answer": str(row.get("ground_truth", "")).strip(),
                "category": "",  # No category column
            }
        )
...
def instructions_page():
    ...
    st.title("Image Sequence Rating Experiment (Sahil New)")
    st.write(
        """
        ### Instructions

        1. You will see a series of **questions**, each accompanied by a sequence of **up to 10 images** (video frames).
        ...
        3. After rating all images, answer whether this is a **good datapoint** (Yes / No).
...
def experiment_page():
    ...
    if len(trial["frames"]) == 0:
        st.warning("No frames found for this question.")
    else:
        # Determine number of rows dynamically based on frame count (up to 5 cols per row)
        import math
        num_frames = len(trial["frames"])
        num_rows = math.ceil(num_frames / 5.0)
        
        idx_counter = 0
        for row_num in range(num_rows):
            num_cols = min(5, num_frames - idx_counter)
            cols = st.columns(5)
...
def done_page():
    ...
    part = st.session_state.selected_part
    st.success(f"Thank you for completing the experiment!")
    ...
    if st.session_state.results:
        ...
        st.download_button(
            ...
            file_name=f"results_sahil_new_part{part}_{st.session_state.user_id}.csv",
            mime="text/csv",
        )
...
def main():
    st.set_page_config(
        page_title="Image Sequence Rating Experiment (Sahil New)",
        layout="wide",
    )
